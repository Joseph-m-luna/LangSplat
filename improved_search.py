...
def compare(vec_a, vec_b):
    # Normalize the outputs to unit vectors
    output1_norm = torch.nn.functional.normalize(vec_a, p=2, dim=0)
    output2_norm = torch.nn.functional.normalize(vec_b, p=2, dim=0)
    
    product = output1_norm*output2_norm
    
    # Calculate cosine similarity as dot product of normalized vectors
    cosine_sim = torch.sum(product, dim=0)
    
    # Calculate the loss as the mean squared error from the target similarity
...
def render_set(model_path, source_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        plt.imshow(rendering.detach().cpu().numpy().transpose(1, 2, 0)[:, :, 0:6:2])
        plt.show()
        ...
        model = encoder()
        model.load_state_dict(torch.load("/home/joseph/PRISM/Autoencoder/checkpoints_v8/text_encoder_9.pth"))
        model.to("cuda:0")
        model.eval()

        embedding = clip_lookup("walkway")
        new_embedding = embedding.repeat(64, 1)
        #print("new embedding start size", new_embedding.size())
        new_embedding = model(new_embedding.float())
        print(new_embedding)
        ...
        for chunk in chunks:
            ...
            if chunk.size(1) < 64:
                ...
                new_embedding = model(new_embedding.float())
            decoded = F.cosine_similarity(chunk.transpose(1, 0), new_embedding, dim=1)
            #decoded = compare(chunk.transpose(1, 0), new_embedding)
            proc_chunks.append(decoded)
        
        #reconstruct image
        final = torch.cat(proc_chunks).view(782, 1396).cpu().numpy()
        print("min max:", final.min(), final.max())
        #val = (final.min() + final.max())/2.0 + 0.02
        #val = final.max() - 0.075
        val = 0
        print(val)
        final[final < val] = final.min()
        plt.imshow(final)
        plt.show()

def render_set_high(model_path, source_path, name, iteration, views, gaussians, pipeline, background, args):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "ours_{}".format(iteration), "gt")
    render_npy_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_npy")
    gts_npy_path = os.path.join(model_path, name, "ours_{}".format(iteration), "gt_npy")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        model = encoder()
        model.load_state_dict(torch.load("/home/joseph/PRISM/Autoencoder/checkpoints_v8/render_encoder_9.pth"))
        model.to("cuda:0")
        model.eval()

        embedding = clip_lookup("lush green lawn")
        new_embedding = embedding.repeat(64, 1)
        #new_embedding = model(new_embedding.float())
...
def render_sets_high(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, args):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians, shuffle=False)
        checkpoint = os.path.join(args.model_path, 'chkpnt30000.pth')
        ...
        if not skip_train:
             render_set_high(dataset.model_path, dataset.source_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, args)

        if not skip_test:
             render_set_high(dataset.model_path, dataset.source_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, args)
